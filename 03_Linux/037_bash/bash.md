...
今回は多くのUNIX系OSで標準的に使われるbashシェルについて学習します。
...
ターミナルを起動後、下記のコマンドを実行し、viエディタを起動します。

```bash
vi hello.sh
```

viエディタ起動後、下記のスクリプトを記述して保存します。
...
条件式が真の場合は処理1を行い、偽の場合は処理2を実行します。

![sample_if_else](img/sample_if_else.png)

条件式1が真の場合は処理1を実行し、条件式1が偽かつ条件式2が真の場合の場合は処理2を実行し、それ以外の場合は処理3を実行します。
...
変数aに文字列を代入し、変数aとパターンに指定した文字列が完全に一致した場合、パターンに指定した処理が実行されます。

　　
## 繰り返し処理

スクリプトで繰り返し処理を行うには for文 、while文 を使用します。
