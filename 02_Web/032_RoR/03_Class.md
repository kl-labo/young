...
クラスはclassで始まり、endで終わるように定義します。クラス名はアルファベットの大文字で始めます。一章でも説明した通り、全てのクラスにObjectクラスが継承されているので、newメソッドを使ってインスタンスを作成することができます。
...
インスタンス内のみで参照できる変数です。インスタンス変数名は@で始める必要があります。
Rubyではインスタンス変数への代入を行うメソッド（Javaでいうところのsetterメソッド）は末尾にイコールをつける必要があります。

ここでは受け取った長さ（length）を返すクラスを作ってみましょう。
...
attr_accessorを利用することでgetterとsetterの定義を省略することこができます。
...
クラスメソッドはインスタンス化しなくても呼び出せるメソッドです。メソッド名の前にself.をつけて定義します。オブジェクトを生成するnewメソッドもクラスメソッドです。
...
Rubyでは単一継承をサポートしています。継承は以下の様に<を利用します。サブクラスで同名のメソッドを定義することでオーバーライドすることができます。
